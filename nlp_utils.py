...
:parameter
    :param data: dataframe - dtf with a text column
    :param column: string - name of column containing text
:return
    dtf: input dataframe with 1 new columns
'''
...
    :param column: string - name of column containing text
:return
    dtf: input dataframe with 5 new columns
'''
...
def add_sentiment(data, column, algo="vader", sentiment_range=(-1,1)):
    dtf = data.copy()
    dtf[column] = dtf[column].astype('str')
    ## calculate sentiment
    if algo == "vader":
    ...
    dtf['sentiment_type']=''
    dtf.loc[dtf.sentiment>0, 'sentiment_type']='POSITIVE'
    dtf.loc[dtf.sentiment==0,'sentiment_type']='NEUTRAL'
    dtf.loc[dtf.sentiment<0, 'sentiment_type']='NEGATIVE'
    print(dtf.sentiment_type.value_counts().plot(kind='bar',title="Sentiment Analysis"))
...
    cleaned text
'''
def preprocess_text(txt, lst_regex=None, lower=True, lst_stopwords=None, stemm=False, lemm=True):
    ...
    if stemm is True:
        ...
        lst_processed_words = [ps.stem(word) for word in lst_words]
                
    ## Lemmatization (convert the word into root word) --> do it if you want restore the meaning of the word
    if lemm is True:
        lem = nltk.stem.wordnet.WordNetLemmatizer()
        lst_processed_words = [lem.lemmatize(word) for word in lst_words]

    ## remove leftover Stopwords
    if lst_stopwords is not None:
        lst_txt = [word for word in lst_processed_words if word not in lst_stopwords]
...
    :param column: string - name of column containing text
:return
    dtf: input dataframe with one new columns
'''
def add_preprocessed_text(data, column, lst_regex=None, lower=False, lst_stopwords=None, stemm=False, lemm=False, remove_na=True):
    dtf = data.copy()

    ## apply preprocess
    dtf = dtf[ pd.notnull(dtf[column]) ]
    dtf[column+"_clean"] = dtf[column].apply(lambda x: preprocess_text(x, lst_regex, lower, lst_stopwords, stemm, lemm))
    ...
    if dtf["check"].min() == 0:
        ...
        if remove_na is True:
            dtf = dtf[dtf["check"]>0] 
    dtf.drop("check", axis=1, inplace=True)    
    return dtf
...
#            train_data.append(tupla)
        ...
        ## train
        other_pipes = [pipe for pipe in ner_model.pipe_names if pipe != "ner"] ###ignore alter pipe
...
def fit_w2v(corpus, ngrams=1, grams_join=" ", lst_ngrams_detectors=[], min_count=1, size=300, window=20, sg=1, epochs=30):
    ...
    nlp = gensim.models.word2vec.Word2Vec(lst_corpus, vector_size=size, window=window, min_count=min_count, sg=sg)
